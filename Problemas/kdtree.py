...
# Taken from: <https://rosettacode.org/wiki/K-d_tree#Python>.
...
def random_point(k):
	return [randint(0,1000) for _ in range(k)]
...
if __name__ == "__main__":
    ...
    N = 400000
    for i in range(2,6):
        tupla1Cero = eval("P("+",".join((["0"]*i))+")")
        tupla1Uno = eval("P("+",".join((["1"]*i))+")")
        t0 = clock()
        kd2 = KdTree(random_points(i, N), Orthotope(tupla1Cero, tupla1Uno))
        t1 = clock()
        text = lambda *parts: "".join(map(str, parts))
        show_nearest(i, text("k-d tree with ", N,
                             " random "+str(i)+"D points (generation time: ",
                             t1-t0, "s)"),
                     kd2, random_point(i))
